...
- aadiscordbot (optional, for DM and channel posting features)
    ...
    try:
        ...
        if webhook_url:
            ...
            if response.status_code in [200, 204]:
                ...
                logger.warning(f"Failed to send corp summary. Status: {response.status_code}")
                return False
        else:
            logger.error("No webhook URL or channel ID provided for corp summary")
            return False
...
# Integration example for tasks.py
def integrate_discord_notifications(task_function):
    """
    Decorator to add Discord notification functionality to existing task functions.
    ...
    Example usage in tasks.py:
        ...
        from .discord_integration import (
            ...
            send_corp_tax_summary,
            get_user_discord_id,
            integrate_discord_notifications
        )
        ...
        def notify_taxes_due(self):
            ...
            for u in user2taxes.keys():
                if user2taxes[u][0] > MININGTAXES_PING_THRESHOLD:
                    ...
                    main_char = u.profile.main_character.character_name if u.profile and u.profile.main_character else "N/A"
                    corp_summary_data.append({
                        'username': u.username,
                        'main_character': main_char,
                        'balance': user2taxes[u][0] / 1000000  # Convert to millions
                    })
                    
                    # Send individual Discord DM if enabled
                    if settings.discord_send_individual_dms:
                        discord_id = get_user_discord_id(u)
                        if discord_id:
                            dm_message = f"{message}\\n\\n💡 **How to Pay:**\\nSend ISK to corp wallet with the keyword: `{settings.phrase or 'tax'}`"
                            send_discord_dm(
                                discord_id,
                                title,
                                dm_message,
                    ...
                    # Send public notification to channel if enabled
                    if settings.discord_webhook_url and settings.discord_notify_first_reminder:
                        discord_message = f"**{u.username}** - {message}"
                        send_discord_notification(
            ...
            if corp_summary_data and settings.discord_send_corp_summary:
                channel_id = settings.discord_corp_channel_id if settings.discord_corp_channel_id else None
                webhook = settings.discord_corp_webhook_url or settings.discord_webhook_url
                if channel_id or webhook:
